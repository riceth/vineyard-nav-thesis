...
**Observation.** The 23 test scenes and 721 training scenes were drawn from a single SemanticBLT release, all captured at the same vineyard site across the same growing season (March–June 2024, per the dataset's provenance).

> **Correction (9 August 2026, additive — the observation above stands; its capture dates are wrong).** The scenes were **not** captured March–June **2024**. Capture ran **March–September 2022**.
>
> **Primary source.** Polvara et al. (2024) §3.2: *"we collected a data set on the Ktima Gerovassiliou vineyard (Greece) over an entire crop season, starting in March 2022 until September 2022"*, and: *"11 sessions have been recorded so far in 2022: March 23, April 6 and 20, May 6 and 18, June 1, 8, 22, and 29, July 13, and September 15."* The march bag evaluated here — session **2022-03-23** (`GEOMETRY_PIPELINE_SPEC.md:23`) — is the **first session on that list**.
>
> **Where "2024" came from.** It is the **Roboflow export date, not a capture date.** The dataset directory is `SemanticBLT.v1-2024-june.coco-segmentation`, and in all three split files (`{train,valid,test}/_annotations.coco.json`) **every one of the 1,035 images carries the identical `date_captured` value `2024-06-26T15:45:55+00:00`** — byte-identical to each file's `info.date_created`. A single timestamp shared by every image in a dataset spanning seven months cannot be a capture time; Roboflow stamps the export moment into that field. Verified 9 August 2026 by reading the three COCO files directly (966 train + 46 valid + 23 test = 1,035, one distinct value).
>
> **What does not change.** The within-distribution argument above is unaffected — it turns on the scenes sharing a site and a season, which they do. Only the year and the end month are corrected: **March–September 2022**, eleven sessions, of which this dataset's month-prefixed scenes cover march, april and may (`DECISIONS.md:756`).
...
*Discussion (remediation options — none adopted):* Preliminary calculation: if a mask-area filter (e.g. max_area > 3,000 px on 640×640 images) removed detection #0's 76,837 px mask while retaining all 12 correct detections on 6799, the aggregate Phase B test rasterised fg IoU would rise from 0.556 [0.466, 0.633] to approximately 0.581, narrowing but not closing the gap against Phase A's 0.72 [0.66, 0.77]. The remaining gap reflects consistent under-coverage of thin structures across most test scenes — an architectural effect distinct from the catastrophic false-positive on 6799. Neither remediation is adopted here. A mask-area filter would be principled but hides the failure mode from Phase B's reported numbers; the downstream RANSAC line-fit stage is a more natural location for spurious-input handling if needed. Higher input resolution (e.g. 1280×1280) would reduce mask-head downsampling of thin structures and likely improve under-coverage, but was not explored — it would break the fixed 640×640 input shared by arms A and B. *(Correction, 9 Aug 2026: the original wording called this "the controlled A vs B architectural comparison". A↔B is not a controlled architecture comparison — the arms differ in at least thirteen respects, D006/D021 — so the reason to hold resolution fixed is that it is one of the few variables A and B **do** share, and changing it would add a fourteenth difference. The design choice stands; the justification is narrower than stated.)* Both options are noted for possible future work.
...
> **Amendment (27 July 2026, additive — geometric-stream blob audit; the original 6799 finding stands unchanged).** F007 above is a **perception-evaluation** result: on the labelled SemanticBLT **test scene** 6799, YOLOv11-seg emits a whole-canopy false-positive **mask** (~19 % of frame) at val-fitness `best.pt`, 2/3 seeds per YOLO arm. A distinct question is whether that pathology **reaches the geometric pipeline**, whose base points come from real **bag footage** (in-row eligible frames) behind the D035 15 %-bbox blob guard. Auditing every eligible frame × 3 Phase-C seeds across all three evaluated bags — **~887,000 detections** (march 234k, april 236k, may 416k over 3 seeds) — **zero** exceed the 15 % guard on any bag or seed, **including canopy may** (blob rate 0.00 % everywhere).
>
...
**Citation map.** Ours: `final/march_evaluation/line_fit_report.json` (pooled per-arm tilt + whole-bag m_L/m_R, 12-col schema); `results/geometric/april/final/april_evaluation/line_fit_report.json` (second-bag confirmation); historical val/test in `superseded/march_val_test_split/{val,test}_evaluation/line_fit_{val,test}_report.json`. No paper support needed (measured pipeline property). Mechanism → F017.
...
**Confirmed on a second bag (april; `results/geometric/april/final/april_evaluation/line_fit_report.json`).** Whole-bag two-row coverage A **77.4 ± 0.4** / B **78.2 ± 1.0** / C **76.5 ± 0.2 %** — **arm-independent** (spread 1.6 pp), and this directly **vindicates F011's own bound** below: april lands at ~77 %, *not* the march val's 83–84 %, independently reproducing the held-out-test regime rather than the val regime. The "✗ claim 83–84 % generalises to all bags" caution was therefore correct — coverage is bag-dependent (val 83–84 % / march-test ~77 % / march-pooled ~81 % / april-pooled ~77 %), while the arm-independence is the invariant that transfers. **What april does NOT re-test: the rescued-count / zero-loss claim.** That requires the far-extension-off baseline (rescued = old≠two-row → new two-row), which is a march-specific comparison not re-run here; april confirms the *coverage magnitude and its arm-independence*, not the 871–975-rescued-with-zero-loss figure, which remains a march measurement under the specific D037 gate.
...
- ✗ claim 83–84 % generalises to all bags (march-test ~77 %, **april ~77 % — confirmed bag-dependent**; the arm-independence is what generalises, not the coverage magnitude).
...
**Future work.** **Open observation, flagged not investigated (23 July 2026):** april's GT-1 RMS is **~20 % lower** than march's (per-arm 0.164 / 0.175 / 0.173 m vs march's 0.209 / 0.215 / 0.215 m) despite **~4 pp *less* two-row coverage** (77–78 % vs 81 %) and slightly *fewer* mean base points (24.7–27.6 vs 26.2–31.5) — i.e. fewer fits, but better ones. This is **not explained here.** The right instrument is F012's own regression-residual decomposition re-run on april (angle-to-corridor re-derived from `/robot_pose`), which would show whether the improvement lives in the bias term, the residual SD, or both; until that runs, no mechanism is claimed and the cross-bag RMS difference must not be read as a pipeline improvement. **(Update, 23 July 2026):** F025's april sweep shows this lower RMS is not cosmetic — it shifts the near-seed tuning optimum (Opt-A vs Opt-B diverge in direction vs march), so the decomposition matters for tuning, not only for reporting. Still not run; still flagged.
...
**Confirmed on a second bag (april; 8,889 in-row frames, ~6,200 both-two-row per pair; block lengths re-derived on april — L_GT1 = 11, L_GT2 = 28, vs march's 9/20).** GT-1 remains indistinguishable: A-B **−3.2 mm [−5.7, 0.0]**, A-C **−2.2 mm [−4.6, +0.5]**, B-C **+1.0 mm [−0.7, +2.3]** — all ≤ 3.2 mm (≤ 8.3 % of the RTK floor), all CIs include zero. **The A-B interval closes *at* zero at the reported precision** rather than spanning it comfortably; it is the one marginal pair and is reported as such, not as a clean null. **The march GT-2 sub-noise-floor offset does *not* reproduce** — see the amendment note below.

**Confirmed on a third bag (may; canopy season; 26 Jul 2026; 8,840 in-row frames, ~4,600–4,740 both-two-row per pair; block lengths re-derived on may).** Per-arm cross-seed: A two-row 67.6±1.1%, GT-1 0.231±0.002 m, GT-2 4.17±0.04°; B 65.1±2.7%, 0.241±0.005 m, 4.16±0.04°; C 63.0±1.2%, 0.243±0.002 m, 4.16±0.03°. For the first time across the evaluated bags, two **GT-1** contrasts exclude zero — **A-B −4.1 mm [−7.5, −1.1]**, **A-C −5.4 mm [−9.2, −2.6]** — but both are **sub-floor** (≤ 14.3 % of the 3.8 cm RTK floor) and **arm-A-specific**; **B-C stays indistinguishable (−0.5 mm [−2.3, +0.6], CI includes zero)**. On GT-2 only A-C excludes zero (+0.064° [+0.007, +0.131], 4.9 % of the ~1.3° noise floor); A-B and B-C include zero. **This is not a larger-n effect** — may's per-pair n (4,588–4,737) is *smaller* than march (~5,800) / april (~6,200), because canopy lowers coverage; the A-vs-{B,C} GT-1 gap is genuinely marginally larger on the canopy bag (−4/−5 mm vs march ≤ 1 mm / april ≤ 3.2 mm), not a precision effect. **The navigation-relevant conclusion holds on all three bags** — every difference is well below the measurement floor. Two refinements, in the same honest register as the pooled GT-2 offset above: (i) may is the first bag where the *primary* GT-1 metric statistically resolves a sub-floor difference (previously only GT-2 did, on march pooling); (ii) it is **arm-A-specific** (the system-level A↔B / A↔C contrasts, R55/F008), while the **B↔C class-structure ablation remains null on both metrics** — no class-structure effect on a third bag, the cleanest confirmation of F013's core claim. Artefact: `results/geometric/may/final/may_evaluation/paired_crossarm.json`.

**Confirmed on a fourth bag (june; canopy; 9 August 2026; block lengths re-derived on june).** GT-1 paired differences are **A–B −2.3 mm, A–C −0.4 mm, B–C +1.7 mm** — all ≤ 2.3 mm, ≤ 6.1% of the 3.8 cm RTK floor. GT-2: **A–B −0.042°, A–C −0.027°, B–C +0.009°**, all ≤ 3.2% of the ~1.3° noise floor. **Every one of the six sign patterns is inconsistent across seeds** (`− + −`, `− + −`, `+ + −` on GT-1; `+ − −`, `+ − −`, `+ + −` on GT-2) — no pair holds its sign on either metric.

**This is the cleanest null of the four bags, and it lands where a real effect should have been easiest to see.** May resolved two sub-floor GT-1 contrasts (A–B −4.1, A–C −5.4 mm) and june is also canopy, with comparable coverage (67.8/64.9/61.5% vs may's 67.6/65.1/63.0). May's two resolved contrasts shrink by factors of **1.8 and 13.5** on june and neither is sign-stable; **B–C, which resolved on neither bag, remains within ±2 mm on both**. **May's marginal A-specific separation therefore does not reproduce on a second canopy bag**, which bounds it as session-specific rather than a canopy property, exactly as april bounded march's GT-2 offset.

**B–C — the controlled comparison — is null on all four bags**, on both metrics, without exception. Artefact: `results/geometric/june/final/june_evaluation/paired_crossarm.json`.
...
**April (second bag):** `results/geometric/april/final/april_evaluation/paired_crossarm.json` (block lengths re-derived per-bag via `block_lengths.py` — april's decorrelation is longer than march's, so march's blocks are **not** reusable). Per-arm across-seed CIs all overlap (A n=6,602 GT-1 0.1525 [0.1441, 0.1592] m / GT-2 2.232 [2.111, 2.312]°; B n=6,505 0.1567 [0.1450, 0.1604] / 2.210 [2.076, 2.288]; C n=6,390 0.1570 [0.1445, 0.1590] / 2.213 [2.082, 2.283]).
...
> **Amendment (23 July 2026, additive — april second-bag test; F013's march numbers and conclusions above stand).** The sub-noise-floor GT-2 offset that pooling resolved on march (ordering A < C < B; all three pairs' CIs excluding zero at the primary block) **does not reproduce on april**: A-B **−0.004° [−0.067, +0.039]**, A-C **−0.009° [−0.066, +0.034]**, B-C **−0.006° [−0.024, +0.017]** — every CI includes zero at the primary block **and** under the stricter 0.05-threshold block (L = 54), and the sign is **inconsistent across seeds for all three pairs** (march had A-C sign-consistent). This **bounds the march offset as a scene/session-specific artefact rather than an arm-level property** — which is what F013's own hedge suspected ("not attributable to an arm-level mechanism at this magnitude") but could not demonstrate from one bag. The qualification on the GT-2 claim is therefore narrowed, not widened: it is a march observation, not a property of the arms. Artefact: `results/geometric/april/final/april_evaluation/paired_crossarm.json`.
...
- ✗ claim GT-2 indistinguishability unconditionally **on march** (pooled: a sub-noise-floor offset persists there — A–B and A–C exclude zero robustly). Equally: ✗ carry the march GT-2 ordering to other bags — it does not reproduce on april (amendment above).
- ✗ present the april A–B GT-1 interval as a clean null (it **closes at zero**, [−5.7, 0.0] mm).
...
**Whole-bag cross-check (val→test→pooled).** The sensor-common tilt was measured across three anchor sets, all consistent in the operative claim (LiDAR nonzero, same sign as the camera): **val** (6 anchors) LiDAR +3.84° (SD 0.17°) / camera +3.25°; **confirmed on the held-out test** (6 anchors, 2 per test corridor) LiDAR +3.04° (SD 0.36) / camera +2.74° (Δ −0.31°); and the **pooled whole-bag cross-check** (10 anchors, 2 per corridor across all five corridors; `final/march_evaluation/lidar_crosscheck.json`) LiDAR **+2.57° (SD 0.96)** / camera **+1.86°**, camera−LiDAR **−0.71°**. **→ These three pooled figures are SUPERSEDED by the amendment below: they were produced by a since-fixed anchor-selection bug (D047) that sampled row-exit frames. Corrected values: LiDAR +2.53° (SD 0.70) / camera +2.29° / camera−LiDAR −0.24°.** The per-anchor magnitude is scene-dependent (the val anchors were high-tilt-selected; the pooled set spans all corridors including the sparse corridor 4, lowering the mean), but LiDAR and camera agree in sign and nonzero-ness on every set — the sensor-commonality that refutes the camera-yaw attribution is invariant val→test→pooled.

> **Amendment (23 July 2026, additive — anchor-selection bug fix; F017's conclusion is unchanged and strengthened).** The whole-bag anchor selector contained a bug: it indexed `fs[len(fs)//3]` into each corridor's **concatenated** frame list, but a corridor is normally driven several times, so that index is not a mid-pass frame. On both bags it landed near a **row exit**, where the measurement degrades on both sensors — LiDAR returns fall to roughly a third of their mid-row count, and the camera row-fit destabilises (cross-model heading SD rises from ~0.50° mid-pass to ~1.07° past 90% of a pass, measured over all april in-row frames). **Two anchors per bag were affected, in march as well as april** — march f8201/f8202 at 97.7/97.9% of pass 4 (LiDAR +0.92/+0.87, camera −0.72/−0.79) and april f1533/f1534 at 95.7/95.9% of pass 0 (LiDAR +2.22/+2.23, camera −0.49/−0.60). In each case the camera heading fell within one cross-model SD of zero, i.e. was statistically indistinguishable from zero rather than genuinely negative.
>
> The selector now groups a corridor's frames by pass, takes the corridor's longest traversal, and samples its true midpoint (`lidar_crosscheck.py`; a second latent bug — a hardcoded PointCloud2 topic id — was fixed in the same pass by resolving the topic by name). Regenerated:
>
> | | march before | **march after** | april before | **april after** |
> |---|---|---|---|---|
> | LiDAR | +2.57° (SD 0.96) | **+2.53° (SD 0.70)** | +2.60° (SD 0.78) | **+2.85° (SD 0.57)** |
> | camera | +1.86° | **+2.29°** | +1.22° | **+2.78°** |
> | camera − LiDAR | −0.71° | **−0.24°** | −1.38° | **−0.06°** |
> | sign disagreements | 2/10 | **0/10** | 2/8 | **0/8** |
>
> **The sign-agreement claim is narrowed** to its defensible form: *camera and LiDAR agree in sign at genuine mid-pass anchors on both bags; near a row exit both sensors degrade and the camera heading is not a reliable sign test there.* The earlier per-frame phrasing was scoped to the hand-picked val 6-anchor set (where it holds); it must not be read as covering the pooled sets, which contained the artefact.
>
> **This strengthens F017 rather than weakening it.** The artefact appears at the *same relative pass position with the same degradation signature in two independently recorded bags*, which is itself evidence of a shared geometric mechanism rather than a bag-specific oddity — the same argument F017 makes for the tilt. And with clean anchors the two sensors agree to **0.24° (march) and 0.06° (april)**, far tighter than the −0.71°/−1.38° the buggy anchors implied. See D047 for the fix and the binding rule for the remaining bags.

**Writeup wording (A2):**

**Fully defensible.** The systematic heading tilt observed in the camera pipeline (mean +2.28°, F010) is also observed by the vehicle's Ouster OS1-16 LiDAR. On six anchor frames, a robust per-side line fit to the trunk-height LiDAR returns (0.2 < Z < 1.2 m), transformed into base_link, yields a mean row heading of +3.84° (SD 0.17°), agreeing with the camera in sign on every frame (camera mean +3.25° on the same frames). Because the LiDAR has an identity base_link→sensor extrinsic in Polvara et al. (2024) Table 3 (quaternion (0,0,0,1)), it cannot carry the camera-mount yaw an earlier hypothesis proposed (F015); that two independent sensors measure a nonzero tilt of the same sign localises the tilt to the geometric relationship between the robot's body frame and the vine rows, rather than to a camera-specific artefact. It is confirmed on the held-out test set (LiDAR +3.04° / camera +2.74°, within 0.31°) and on the pooled whole-bag cross-check across all five corridors (LiDAR +2.57° / camera +1.86°, camera−LiDAR −0.71° — **SUPERSEDED, anchor-selection bug D047; corrected LiDAR +2.53° / camera +2.29° / Δ −0.24°, see the amendment above**). Characterising the tilt as sensor-common — thereby excluding a camera-specific cause — is a contribution of the present work; the paper provides the extrinsics but does not investigate the tilt. (Anchor frames were selected toward high-tilt scenes for visual clarity, giving magnitudes above the ~2.28° pooled mean; the same-sign, sensor-common conclusion is robust to that selection.)
...
**Confirmed on a second bag (april; `results/geometric/april/final/april_evaluation/config_analysis.json`).** The mechanism reproduces in full and **class-agnostic re-locks** (`agnostic_locked=True, pause_flag=False`): in the viable regime (coverage ≥ 70%: agnostic, trunk_T8/T12, pole_T12) all cells' CIs mutually overlap on **both** metrics (`flat_gt1_viable_all_overlap` and `flat_gt2_viable_all_overlap` both True); the viable argmin is again **pole_T12** (GT-1 0.156 [0.144, 0.158]), whose CI overlaps agnostic (0.158 [0.145, 0.161]) → the pre-stated tie-break locks class-agnostic. Trunk-only matches agnostic on quality (GT-1 0.161 [0.149, 0.169] vs 0.158 [0.145, 0.161], CI-overlapping; GT-2 2.31 vs 2.21°) while giving up coverage, so poles supplement **+13.6 pp** (agnostic 76.5% vs trunk-only 62.9%) — closely matching march's +12.4 pp — at no measurable quality cost. Pole-only degenerates identically (**0.6% two-row, n=1 frame**), and the pole cells climb out of degeneracy only by trunk-fallback (pole_T5 6.8%, pole_T8 52.0%, pole_T12 75.6%). The **trunks-load-bearing / poles-supplement-coverage-not-quality** conclusion therefore holds on a second, independent bag — still at bare-vine density; whether poles carry more signal under canopy remains the O019-gated question F018 already registered.
...
**Caveats.** The config-invariance of centreline quality is measured at F013's bounds (val, held-out test, and pooled whole bag). A denser-canopy bag (different trunk/pole visibility) is a multi-bag question — poles may carry more structural information at higher visibility. The config-invariance of centreline quality is now measured on **two** bags (march + april, both bare-vine); the denser-canopy question is unchanged and O019-gated.
...
**Confirmed on a second bag — with one claim narrowed (april; 15,156 non-in-row frames, 9 models; `results/geometric/april/final/non_in_row_evaluation/non_in_row_analysis.json`).** The *headline* reproduces and, on turns, strengthens: the pipeline invents a `two_row` centreline on **79.7–80.6 %** of turn frames (march 76.0–80.5 %) — the U-turn-facing-down-a-corridor mechanism — now **arm-consistent to <1 pp**. Transition also reproduces (47.9–50.0 %, march 49.0–53.0 %).

**But F020's "arm-consistent (≤4 pp) pipeline property, not an arm one" claim FAILS on the stationary category.** On april's stationary frames the spurious rate is **A 12.9 / B 20.5 / C 27.4 %** — a **14.5 pp** spread, monotonically ordered A < B < C, versus march's arm-consistent 44.9 / 49.2 / 48.0 % (≤4 pp). This is **the first material output-level divergence between the three arms anywhere in the project.**

**Scope of that divergence — stated up front so it is not misread.** It is a **non-in-row robustness property, not a navigation-quality one, and it does not touch F013.** F013's in-row indistinguishability (GT-1 ≤ 3.2 mm paired, both bags) is unaffected: this is a difference in how readily each arm *hallucinates a centreline while stopped over out-of-distribution headland input*, not a difference in centreline accuracy where a real row exists. No in-row metric moves.

**Mechanism: not determined, and specifically NOT a base-point-count effect.** The obvious explanation — "the arm that detects more base points fits two spurious rows more often" — is **refuted by the data**: on these same stationary frames mean base points are A 21.5 / B **23.4** / C 22.2, so B detects the *most* yet sits in the *middle* of the two_row ordering. The divergence is arm-ordered A < B < C but does not track detection count; its cause (plausibly a class-structure difference in how the multiclass arm's detections cluster into two sides on ambiguous stationary scenes) is **not established here** and is left as an observation, not a claim.

**Why the overall rate also drops (48–52 % march → 20–32 % april).** April's non-in-row stratum is **81 % stationary** (12,334 / 15,156) vs march's 68 %, dominated by the ~397 s mid-session idle period (D046a) during which the robot pointed at low-row-structure scenes; the large low-rate stationary mass drags the pooled rate down. The overall figure is therefore a **composition-dependent quantity**, not a stable pipeline constant — the per-category rates (esp. turn) are the transferable ones.
...
- ✗ extend "arm-consistent, a pipeline property" to the **stationary** category (april: 14.5 pp, A<B<C) — it holds for turn/transition only; the stationary spurious rate is arm-dependent, though this is a non-in-row robustness property that does not touch F013.
- ✗ read the overall spurious-two_row % as a stable constant (48–52 % march / 20–32 % april — it depends on the bag's non-in-row composition; the per-category rates transfer, the pooled rate does not).
...
**Confirmed on a second bag (april; `results/geometric/april/final/non_in_row_evaluation/non_in_row_analysis.json`).** The driven-path error reproduces: RMS lateral **0.336–0.380 m** (A 0.380 / B 0.363 / C 0.336) and RMS heading **5.2–6.5°** — again ~2× the in-row centreline error (F013, april ~0.16–0.17 m) and arm-similar, a degradation characterisation not comparable to the in-row metric. The magnitude sits slightly below march's 0.40–0.43 m, tracking april's tighter in-row RMS (the F012 open observation), not any change in the projection breakdown. The two_row denominator is larger and arm-varying (n = 9,128 / 12,082 / 14,602) — a direct consequence of F020's stationary arm-divergence, not a property of this metric.
...
> **Superseded in part (9 August 2026, additive — the geometric-strand finding below is unchanged and stands).** For the **control strand only**, D042 replaces this gate's signal source: the state gate there reads the **native bag twist**, not the pose finite-difference used here, and D042's own 20 July 2026 amendment further corrects the gate *predicate* (forward `v_x`; the turn predicate dropped). F022 as written remains the geometric-strand result. This note was added because the supersession was recorded only in D042 and a reader arriving at F022 directly had no way to see it.
...
**Confirmed on a second bag — arm-invariance holds *per category*, overall closure is lower (april; `mitigation_analysis.json`).** F022's causal rejection is **per-category arm-invariant**, exactly as on march: stationary **100 %** (all arms), turn **87.6–87.9 %**, transition **69.9–70.7 %** — each row flat across A/B/C. What changes is the **overall** rate: A 87.2 / B 90.1 / C 91.7 % (march 98.4–98.5 %), at a lower in-row FP of **0.9 %** (march 1.2 %).

**The apparent overall arm-variation (87→92 %) is a MIX effect, not a gate change — read this carefully.** The gate is odometry-driven and its per-category behaviour is identical across arms (above). The overall number varies only because the *denominator* — each arm's set of spurious two_row outputs — differs by arm (F020's stationary divergence: C emits more spurious stationary two_row, A fewer), and the stationary category is 100 %-rejected. So the arm with more of its spurious mass in the trivially-caught stationary bin (C) posts a higher overall rejection. **The gate does not behave differently per arm; the population it is applied to does.** Reporting this as "the gate is arm-dependent" would be wrong.

**The genuine drop is the transition category: ~70 % vs march's ~96 %.** April's between-corridor transitions keep the robot moving forward, so a speed/velocity-based state gate does not fire on them — the same body-vs-world-frame limitation F026 identifies (see F026). Combined F022 ∪ F023 closure is therefore **87.8 / 90.4 / 92.0 %** (march 98.6 %) at **2.6–3.4 %** in-row FP (march 3.9–4.7 %). F023's perception-only ceiling reproduces (~27–43 % non-in-row rejection, arm-varying because it operates on the arm-divergent output) with its turn-blindness intact.

**Implication.** The deployment gap F020/F021 measured is **almost entirely closable with odometry the robot already carries** — the missing piece was never perception, it was a state input; a real deployment gates the row-follow controller on this state (the "state machine" F020 concluded). **The ~98 % figure is march-specific, however: on april the same gate closes 88–92 %, the shortfall concentrated entirely in the transition category (~70 % vs ~96 %). The closure percentage is bag-dependent (a function of how prevalent and how forward-moving each bag's transitions are); the gate's per-category behaviour is not.** **Honest limit: F022 requires runtime odometry; if odometry is unavailable or degraded, F022 fails**, and the perception-only fallback (F023) is all that remains.
...
**Confirmed on a second bag (april).** F023's perception-only ceiling reproduces — non-in-row rejection ~27–43 % (arm-varying because it operates on the arm-divergent output, F020), turn-blindness intact — and the combined F022 ∪ F023 closure (87.8–92.0 %). The full second-bag numbers are reported in **F022's April block** (the two layers share one `mitigation_analysis.json` and are evaluated together), not duplicated here.

> **Amendment (26 July 2026, additive — may third-bag; F023's march/april numbers stand). The fixed geometry-filter thresholds do not transfer cleanly to canopy.** F023's thresholds (`|offset| > 0.71 m`, `|heading| > 6.7°`, `|m_L − m_R| > 0.22`, `n_base < 12`) are fixed at march's bare-vine in-row p99 and applied unchanged to every bag. On the canopy bag (may) this raises the geometry filter's **in-row false-positive rate to 9.1 / 7.5 / 8.1 % (A/B/C)** — ~3× march (2.7 / 3.4 / 3.5 %), ~4× april (1.8 / 2.6 / 2.5 %) — driven mainly by **sparse base points (`n_base < 12` fires on 4.6 % of valid in-row frames) and noisier heading (3.9 %)** under foliage, where valid in-row geometry sits closer to the bare-vine rejection thresholds. The **state gate (F022) is unaffected — 0.4 % in-row FP on may** (below march 1.2 % / april 0.9 %), as it uses odometry, not perception. **Implication:** F023 as a perception-only fallback is season-sensitive — a canopy deployment would need per-season or coverage-adaptive thresholds, not the fixed bare-vine set; F022 (odometry) transfers unchanged. This is a bounded characterisation of F023's cross-season transfer — not a change to F022 or the F013 in-row headline. Artefact: `results/geometric/may/final/mitigation_evaluation/mitigation_analysis.json` (`F022_F023_causal.in_row_FP`, `F023_in_row_threshold_overlap`).
...
**Confirmed on a second bag (april; 12,376 single_row model-frames across 9 models; `results/geometric/april/final/april_evaluation/single_row_analysis.json`).** The abstention mechanism reproduces in full: `single_row` on **14.9 / 15.1 / 16.4 %** of in-row frames (A/B/C), dominated by **`too_few_near_seed` at 71.2–72.8 %** — the same near-field-density criterion, at the same dominance — with `too_few_total` 16.4–20.3 %, `abs_y_too_large` 4.9–5.7 % and `horizontal_cluster` 2.1–5.8 %. Determinism holds exactly: **`not_reproduced` = 0 of 12,376**.

Two quantitative shifts, both consistent with april's sparser near field (mean base points 24.7–27.6 vs march's 26.2–31.5): the abstention **rate is 1.5–2.5 pp higher**, and the failing row is **detected more often** (98.2–99.0 % vs march's 96.3–96.8 %, with `not_seen` falling from ~3.4 % to ~1.5 %). The pipeline therefore abstains *more* on april while *seeing* the second row more reliably — reinforcing F024's core claim that the guard tests near-field **evidence density**, not row visibility.

**One qualification.** March's "left/right fits are balanced (no side bias)" is **weaker on april**: right-side failures exceed left by 13–19 % (A 2,157 R vs 1,812 L; B 2,184 vs 1,846; C 2,325 vs 2,052), against march's 3–14 %. Reported as an observed asymmetry, not attributed — it is arm-consistent, so it is a scene/geometry property rather than a perception one, but its cause is not investigated here.

**Arm-consistency** holds at 1.5 pp (march ≤1.1 pp), still far below any level that would support ranking.
...
- ✗ claim left/right abstention balance as a general property (march ≤14 % asymmetry, april 13–19 % right-biased).
...
**Confirmed on a second bag (april; `results/geometric/april/final/april_evaluation/near_seed_sensitivity.json`; NEAR=5 slice reproduces the committed CSV, 0 mismatches).** Every qualitative claim holds: widening from 5 m recovers **25.1–26.4 %** of abstentions at 6.0 m (28.2–32.0 % at 6.5 m), coverage rises from 76.6–78.1 % to a **peak ~82–83 % at 6.5 m then declines** (to 78–81 % at 10 m), full-set RMS rises **monotonically**, and the cost is again a **heavy-tail adjacent-row corruption** — existing-shift median 0 / mean 0.8–1.5 cm but **max 1.55–1.85 m** at 6.5 m (the same catastrophic-capture signature, on ~1.4–2.3 % of fits). Recovered-frame RMS 0.24–0.31 m (~30 % worse than baseline), plausibility-fire 3.4–7.1 %. Arm-consistent (optimal window within ~1 m across A/B/C).

**One structural difference, and it traces to F012's flagged observation.** The two optimisation criteria, which agreed on march (Opt-A 6.5/7.0/6.5 m ≥ Opt-B 6.0 m), **diverge in direction on april**: Opt-A = 5.5 / 7.0 / 6.5 m, Opt-B = 6.5 m for all arms — so for arm A the *aggressive* Opt-A now picks a *narrower* window than the marginal-return Opt-B. This is a direct consequence of april's ~20 % lower baseline RMS (0.164–0.174 m vs march's 0.209–0.215 m; the F012 open observation): Opt-A's 10 %-of-baseline tolerance is a smaller *absolute* cm budget on april, which arm A exhausts by 5.5 m, whereas Opt-B's absolute coverage-pp-vs-RMS-cm comparison rewards april's steep 6.0–6.5 m coverage gain. **The finding is unchanged — 5 m is conservative but near-optimal, a modest widen to ~6 m is defensible, wider is not better — but the specific optimal window is bag-dependent through the baseline RMS, which the march-only F025 could not have shown.** This is exactly the "re-run the sweep on other months" future-work item F025 registered, now discharged for april.
...
**Confirmed on a second bag — and F026 predicted its own failure mode before this data existed (april; `results/geometric/april/final/mitigation_evaluation/state_gate_native.json`).** Every *structural* claim reproduces: (1) the gate collapses to a **single forward-speed predicate** — the turn predicate remains dead (adds +1–3 non-in-row rejections for +72–87 in-row false positives → dropped); (2) the **spec-3 frame error** holds — a literal `|v_y|>0.30` retains only **1.3 %** of in-row frames (march 1.5 %), confirming body `v_y` is lateral slip; (3) the **odom yaw-rate is unreliable** — corr(odom,IMU) −0.46, odom p99 **12.25°/s** vs IMU **3.84°/s** (march −0.43 / 13.4 / 3.5). The threshold `V_MIN` was **re-fit per bag** to the april in-row p1 = **0.348 m/s** (march 0.296), per the D046 rule that bag-measured constants are re-derived, not carried.

**The methodological point: F026's committed text named this exact limitation in advance.** F026 stated the transition shortfall is "a genuine body-vs-world-frame limitation … body forward `v_x` stays high, so the body frame cannot flag transitions the world frame could." That was written on march, where the shortfall was ~4–8 pp. **April, with more and more-forward-moving transitions, makes it large — transition rejection falls to 72.0–72.9 %** (march ~92 %), dragging overall closure to **87.7–92.3 %** at 0.7 % FP (march 97.6 % / 0.9 %). The finding therefore *predicted the shape of its own second-bag degradation before the second bag was run* — the mechanism is confirmed and the deployment-gap closure is shown to be **bag-dependent through the transition composition**, not a fixed ~98 %. Arm-invariant per category throughout.

**Implication.** The F022 deployment-gap closure **transfers to the deployable signal** — the same ~98% rejection at ~1% FP holds on the causal, onboard twist a real controller reads, without the offline whole-trajectory 15-sample centred (non-causal) reconstruction — validating D042's premise. Two corrections were **adopted** into the locked control-strand design (D042 amendment + PID_PIPELINE_SPEC.md §3, 20 Jul 2026): **(i)** the native gate is **corrected to a single forward-speed predicate** (`v_x`, not the body `|v_y|` of the original spec §3) with the **turn predicate dropped** (it adds 0 marginal rejection and only false positives natively); **(ii)** the transition shortfall is a genuine body-vs-world-frame limitation, not a threshold-tuning issue — closing it would need the original-corridor heading (e.g. IMU-integrated yaw or map frame), out of scope for the runtime twist gate. **This shortfall is bag-dependent: ~4 pp on march but ~28 pp on april (transition rejection 72 % vs 92 %), because april's transitions are more prevalent and more forward-moving — so the gate's overall closure is a function of each bag's non-in-row composition, while its per-category behaviour is invariant.** The odom-IMU disagreement is a **sensor-reliability caveat that contrasts F017** (where camera and LiDAR *agreed* on the tilt): here two onboard yaw sources disagree, so a deployed turn detector should prefer the IMU gyro — but the row-follow state gate does not need either.
...
**Confirmed on a second bag — the degeneracy is a property of the BLT navigation mode, not of march (april; `results/geometric/april/final/command_evaluation/gain_kfold.json`).** Every element reproduces and, if anything, deepens: all **12** pass-level folds select the identical near-zero corner (Kp=0.01, Kψ=Kd=Ki=0); the pooled out-of-fold RMS (0.022734 rad/s) beats the zero-gain baseline (0.022781) by only **0.204%** (march 0.378%); and the best-possible linear map centreline→ω_exec explains **R²=0.00445** of the executed yaw-rate (march 0.00698), falling to **0.00001** against odometry (march 0.00057). The arms are again indistinguishable (per-arm OOF spread 6×10⁻⁵ rad/s; per-arm R² 0.0036–0.0054). Because the BLT platform ran the same GPS/topological navigation stack on april, its executed yaw-rate is equally — marginally more — uncorrelated with the perceived centreline, exactly as the mechanism predicts. **This establishes that the tracking-objective degeneracy is a property of the reference platform's navigation mode, not a march-specific artefact, and that the P-4c pivot to first-principles gains was the correct general response, not a one-bag workaround.**

**Two caveats, both stated rather than hidden.** (i) April's k-fold uses **12** pass-level folds including the partial p4 and the threshold-split p7/p8 (D046a), so its folds are less balanced than march's 11 — but the degeneracy is so complete (all 12 folds identical, grid monotonically worse as gains grow) that fold imbalance cannot affect the conclusion. (ii) April's `sim_equivalence_check` ran against the **CP-P2 placeholder gains**, not the P-4c locked gains, because `command_summary.json` did not yet exist when k-fold ran (command_generator runs next); this does not bear on the degeneracy, which is a property of the objective, and the P-4c equivalence is re-checked in F027-A below.
...
**Confirmed on a second bag — the design point holds; the gains are re-derived per bag, not carried (april; `results/geometric/april/final/command_evaluation/command_summary.json`; sim-equivalence to `command_generator.py` at the P-4c gains = 5.02×10⁻⁷ rad/s, matching march's 5.1×10⁻⁷).** Every constant is re-measured from april (D046): in-row v = 0.7724 m/s, offset RMS 0.1654 m, ω_max (IMU p99) 0.06708 rad/s, observed yaw SD 1.383°/s. The **design point reproduces**: 20 m settling at ζ=1 saturates only beyond the p99 offset tail (1.033 m, march 0.937 m), fresh-frame saturation is **0.51%** (march 0.83%), and the derived ramp bound (0.037563 rad/s², march 0.033743) clips only the jitter tail — median demanded slew below the bound, 36.3% of transitions clipped (march 37.9%). The locked gains **Kp = 0.06496, Kψ = 0.00782** land almost exactly on march's (0.06465 / 0.00778) — not because the derivation is bag-invariant but because april's in-row speed nearly matches march's (0.772 vs 0.769 m/s) and the gains are speed-driven; the yaw-envelope constants (ω_max, ramp rate) correctly differ.

**The gentle-reference corollary holds, with a bag-dependent multiple.** A 5 m settling distance would demand 9.85°/s at april's offset RMS = **7.12× the observed in-row yaw SD** (2.56× p99) — lower than march's 10.16× because april's yaw SD is larger and its offset RMS smaller (both F012-linked), but still many times the envelope the platform ever used. The qualitative result — the BLT reference was a gentle, non-vision-reactive controller, and d_s=20 m is bounded from both sides by the platform's own behaviour — reproduces; the specific multiple is offset-RMS/yaw-SD dependent and therefore bag-specific, as expected.
...
### F028 — Command smoothness is arm-indistinguishable; the hold-last cost is real but bounded, and the ramp layer dominates it

**Finding.** On the locked first-principles command stream, **command smoothness does not separate the three perception arms**. Every paired cross-arm difference in RMS frame-to-frame yaw-rate change spans zero — exclusive A−B **−4.2×10⁻⁴ [−1.9×10⁻³, +5.7×10⁻⁴]**, A−C **−3.4×10⁻⁴ [−1.6×10⁻³, +8.9×10⁻⁴]**, B−C **+6.7×10⁻⁵ [−5.3×10⁻⁴, +1.1×10⁻³]** rad/s per frame — and the same holds inclusive. This **converges with the geometric strand** (F013: centreline RMS ~19 cm, arm-indistinguishable): two independent measurement levels agree that binary-vs-multiclass perception does not separate at the navigation output. Three secondary results carry more information than the null. **(i) The dual metric (D043) works as designed and matters:** the inclusive view **under-states** jitter by **2.0–4.7%** relative to exclusive, because **18.4–20.4% of consecutive frame pairs have Δω̂ = 0 by construction** — a held command repeats the previous value, so abstention makes the controller look *smoother*, not rougher. Reporting only the inclusive number would have flattered the result. **(ii) The cost of hold-last is the re-entry jump, not the hold itself:** leaving a hold span produces an RMS Δω̂ of **0.019–0.021**, i.e. **2.0–2.3× a normal control step**. **(iii) The ramp layer dominates everything else**, cutting jerk **4.7–5.3×** (0.129–0.144 → ~0.027 rad/s²) and compressing the hold-exit jump with it. Saturation is **0.37–0.48%** of fresh frames. Under the ramp layer only, a **negligible-but-detectable** B-vs-C micro-difference appears (exclusive **+9.0×10⁻⁵ [+1.6×10⁻⁵, +2.0×10⁻⁴]**, CI excluding zero but ~4% of the metric scale) — the same character as F019's GT-2 micro-difference, and not a basis for ranking.

**Evidence.** `final/command_evaluation/command_smoothness.json` (`scripts/control/command_smoothness.py --bag march`; deterministic — identical md5 on re-run). Metrics per §7b (RMS Δω̂, jitter = SD Δω̂, saturation rate); deltas within-pass only (controller state resets per pass); point estimates over all in-row frames, per-(arm,seed) CIs over the Δs = 1.5 m subsample (n = 284), per-arm and paired CIs by moving-block bootstrap at **L = 20**, B = 10,000, seed 42.
- **Per arm (exclusive, rad/s per frame, mean ± SD across 3 seeds):** A 0.008535 ± 0.00028 · B 0.009542 ± 0.00085 · C 0.009023 ± 0.00020. Inclusive: 0.008138 / 0.009159 / 0.008840.
- **Hold-exit:** A 0.019151 · B 0.019057 · C 0.021042 (2.24× / 2.00× / 2.33× the exclusive value).
- **Ramp layer (exclusive, rad/s²):** 0.12881 → 0.02764 (A), 0.14350 → 0.02688 (B), 0.13529 → 0.02664 (C).
- **Saturation:** exclusive 0.37 / 0.48 / 0.39 %; inclusive 0.54 / 0.73 / 0.73 %.
- **Hold spans:** 122–147 frames maximum (**8.3–10.0 s**) across streams; **2.9–5.7%** of in-row frames sit inside spans longer than 5 s.
- **Jitter ≈ RMS Δω̂ throughout**, because the mean frame-to-frame change is ≈ 0 over a pass; the two locked metrics are near-redundant on this data.
- **Blob-seed observation (descriptive, not inferential).** Within B, the blob seeds (42, 43) are *smoother* than the clean seed (Δ = −1.5×10⁻³); within C, the blob seeds (43, 44) are *rougher* than the clean seed (Δ = +3.9×10⁻⁴). **Opposite signs — no consistent relationship.** n = 3 seeds per arm, and the link is indirect (F007's blob is a whole-frame false mask on one labelled perception test scene, whereas this stream runs over the bag's in-row frames), so this is an observation, not evidence either way.

**Confirmed on a second bag — the convergent null reproduces (april; `results/geometric/april/final/command_evaluation/command_smoothness.json`).** Command smoothness again does not separate the three arms: every raw-command (exclusive) paired cross-arm CI spans zero — A−B **−3.8×10⁻⁵ [−1.4×10⁻³, +6.4×10⁻⁴]**, A−C **−1.7×10⁻⁵ [−1.5×10⁻³, +5.8×10⁻⁴]**, B−C **+2.3×10⁻⁵ [−4.7×10⁻⁴, +5.1×10⁻⁴]**. **Combined with F013's second-bag confirmation, the convergent null — perception differences do not survive to the navigation output at *either* the geometric or the command level — now holds across two seasons**, a materially stronger statement than the march-only version.

The three secondary results reproduce: (i) the D043 **dual metric still matters** — the inclusive view understates jitter by 2.9–4.2% (march 2.0–4.7%) because held commands repeat the previous value; (ii) the **hold-last cost is again the re-entry jump**, 2.32–2.57× a normal step (march 2.00–2.33×); (iii) the **ramp layer again dominates**, cutting jerk 3.4–3.9× (march 4.7–5.3×) — a smaller multiple only because april's raw commands are already smoother (lower offset RMS → lower raw jerk 0.104–0.116 vs march 0.129–0.144), not because the limiter does less; the *ramped* jerk is comparable (~0.030 vs ~0.027). Saturation is even lower (0.07–0.08% vs march 0.37–0.48%), tracking april's tighter offset RMS.

**The ramp-layer micro-difference reproduces in *character* but not in *which arms* — confirming it is not a ranking.** As on march, under the ramp layer only, exactly one paired CI excludes zero by a hair — but it is a **different pair**: march's was B−C (+9.0×10⁻⁵ [+1.6×10⁻⁵, +2.0×10⁻⁴]), april's is A−B (+6.1×10⁻⁵ [+1×10⁻⁶, +1.3×10⁻⁴]). The effect is ~4% of the metric scale on both bags and switches which arms it implicates between them — so, exactly like F013's GT-2 offset (which likewise failed to reproduce its march ordering on april), it is **metric-resolution noise surfacing under smoothing, not a stable arm difference.** No arm ranking is supported at either measurement level, on either bag. (The blob-seed set is unchanged — B seeds 42/43, C seeds 43/44, a fixed checkpoint property of F007 — and remains descriptive-only.)

**Implication.** The command-level strand closes with a **null that is informative rather than empty**: after perception differences propagate through an identical geometric pipeline and an identical controller, they do not survive into steering-command smoothness. Combined with F013 this is **convergent evidence** that the multiclass-vs-binary distinction does not reach the navigation output on this data — a stronger statement than either strand alone, because the two use different measurement levels and different failure modes. The secondary results are the practically useful ones: abstention **flatters** naive smoothness metrics (so any deployment reporting jitter must state whether held commands are counted), the real cost of hold-last is concentrated in the **re-entry transition**, and a rate limiter derived purely from the closed-loop design point removes ~5× of the jerk without any tuning. The P-1 cross-strand check (does perception pathology propagate into control instability?) returns **no consistent signal** and, per its original framing, does not become a finding.

**Cross-references.** D014 (strand-3 smoothness — this is its deliverable); D043 (dual metric, whose value this quantifies); F013 (geometric arm-indistinguishability — the convergence check, §7d); F027 (why the tracking objective is absent here); F027-A (the locked gains and ramp limit); F019 (precedent for a negligible-but-detectable micro-difference); F007/O009 (blob seeds); F024 (abstention, the source of held frames); P-1 (planned cross-strand check); §7b/§7c.

**Writeup wording (A2):**

**Fully defensible.** Command smoothness does not separate the three perception arms: every paired cross-arm difference in frame-to-frame yaw-rate change has a confidence interval spanning zero, whether or not held commands are counted. This agrees with the geometric strand's finding of indistinguishable centreline error, so two independent measurement levels give the same answer. Three subsidiary results are more informative than the comparison itself. Because a held command repeats the previous value, roughly a fifth of consecutive frame pairs have zero change by construction, so counting held frames *understates* jitter by 2–5% — abstention makes the controller appear smoother. The cost of holding is concentrated in the re-entry transition, which is about twice a normal control step. And a rate limiter derived from the closed-loop design point, with no tuning, removes roughly five-sixths of the frame-to-frame jerk.

**Candidate explanations.** The null is expected rather than surprising: all three arms feed an identical geometric pipeline and an identical, untuned controller, so the only route to a smoothness difference is a difference in the per-frame centreline sequence — which F013 already showed is small relative to its own noise. The residual B-vs-C micro-difference under ramp limiting is consistent with B's slightly larger seed-to-seed variance (SD 8.5×10⁻⁴ vs C's 2.0×10⁻⁴), i.e. it may reflect training-run variance rather than a class-structure effect; n = 3 seeds cannot separate these.

**NOT defensible.**
- ✗ rank arms on command smoothness (all paired CIs span zero on the raw command).
- ✗ read the ramped B−C interval as a ranking (magnitude ~4% of the metric; it appears only after smoothing, and B's seed variance is 4× C's).
- ✗ report the inclusive number alone as "jitter" (it is deflated by construction — the dual view is mandatory).
- ✗ claim held commands harm smoothness (they *improve* the naive metric; the harm is the re-entry jump and the stale command itself, not the measured jitter).
...
> **Measurement-basis note (9 August 2026, additive — no number changes).** The pole-supplement figures (+12.4 / +13.6 / +13.8 / +13.7 pp) are **not** computed on the SemanticBLT labelled dataset and share no basis with D018's retention percentages. They come from `config_analysis.json` per bag: the difference in two-row **coverage** between the class-agnostic and trunk-only configurations, measured over **bag video frames** run through the geometric pipeline. There are no augmented variants in bag frames, so the deduplication question that governs D018 (230 unique scenes vs 1,035 augmentation-inclusive rows) **does not arise here** — these numbers need no dedup basis and have none. The two must never be juxtaposed as if they measured the same thing: D018 counts *labelled instances per image* in the training corpus; F029 counts *pipeline coverage on deployment video*. Both are usable in A2; use each for its own claim, and say which corpus it comes from.
...
>
> | dataset | B | C | B−C | ΣSD | sep? | A | B | A−B | ΣSD | sep? |
> |---|---|---|---|---|---|---|---|---|---|---|
> | march | 81.6 | 80.8 | +0.77 | 1.05 | no | 81.3 | 81.6 | −0.30 | 1.42 | no |
> | april | 78.2 | 76.5 | **+1.63** | 1.14 | **YES** | 77.4 | 78.2 | −0.73 | 1.35 | no |
> | may | 65.1 | 63.0 | +2.03 | 3.90 | no | 67.6 | 65.1 | +2.50 | 3.73 | no |
...
**Quantified: most of the disagreement is not vision error.** Combining the two non-vision terms as a conservative direct sum (±182 mm calibration + ~±130 mm per-row reference = **±312 mm**; sum rather than root-sum-square because both are *fixed systematic biases of unknown sign* on a single deployment, not independent random variables — RSS would give 224 mm and understate the envelope), then asking how much of the observed vision–reference disagreement that envelope already accounts for: across **757** publishable two-row frames, **62.0% of the disagreement falls entirely within it**, against **38.4%** within the calibration term alone. Only the remaining **~38%** evidences possible genuine vision error, and that is an **upper bound** — the envelope excludes both the RTK short-scale residual (39–62 mm, D057) and the measurement-zone difference between sites (D058), so widening it to include either can only move frames from the "unexplained" side to the "accounted-for" side. Reproduce with `scripts/riseholme/diagnostics/gt_line_combined_band.py --bag tue02sep` (arm C, seed 42).

This is the strongest available statement that Riseholme's absolute error is **reference-limited rather than vision-limited**, and it is what makes D059's reporting asymmetry a measured position rather than a cautious one: the majority of the absolute error is attributable to sources that cancel exactly in the paired contrasts.

**What this finding is about.** It is a finding about the **reference**, not about the arms. Riseholme cannot rank the arms on accuracy because its reference — a "calculated" mid-row line, RTK at 39–62 mm short-scale, and two unmeasured extrinsic degrees of freedom — is 2–9× coarser than the 19.0–20.2 mm effect being sought (july2023 contrasts corrected 9 Aug 2026; see the D052 correction). This is why D059 makes the paired contrasts primary and the absolute values explicitly caveated.
