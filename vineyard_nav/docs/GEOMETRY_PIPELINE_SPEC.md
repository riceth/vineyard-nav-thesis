...
> **Vine spacing *along* the row — externally sourced, not measured here (11 August 2026, additive).** Previously recorded as not documented. **Papadimitriou et al. (2022), §IV-B, state that "two consecutive trunks are mostly 0.8m apart"** at the same estate (they acknowledge Ktima Gerovassiliou). This is **a figure from a different collection campaign at the same site, not a measurement from this study's data** — no in-row trunk-spacing measurement was made here, and none should be attributed to this work. It is recorded because it bounds the near-seed window's expected trunk count: at ~0.8 m spacing a 5 m window spans roughly six vine positions per side, against the ≥ 2 detections a side needs to seed a fit (D037). Corridor spacing *between* rows (~2.45 m) is measured here and is a separate quantity.
...
> **Terminology amendment (O020, 27 July 2026, additive — the convention name is retained to match committed artefacts).** "Teleoperator-centred" throughout D-F is a naming artefact of the original plan. The BLT platform ran **autonomous GPS/topological navigation** (Polvara 2024 §3.3.3), **not** hand-teleoperation — established by F016 (driven-path framing) and made load-bearing by F027 (recorded steering uncorrelated with vision, R² ≈ 0.007; DECISIONS D014 amendment). Read "teleoperator-centred" as **"driven-path-centred"**: the GT-1 assumption is that the *autonomous run* kept the robot near the corridor centre. Only the characterisation is corrected — the metric, the convention name, and the cross-arm-cancellation argument are unchanged.
