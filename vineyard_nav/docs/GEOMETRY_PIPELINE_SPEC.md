...
5. **Row model (CP-3-locked — D035).** Restrict base points to the **near field X < 5 m** (where the projection fan is smallest, §6) and fit a **Y-constant row** per side: the row position is the **median Y** of the side's points, valid if **≥ 3 points lie within 0.5 m** of that median. Vine rows sit at constant Y in the robot frame, so a constant (not sloped) model estimates the correct quantity — the row's lateral position — and is robust to the projection fan that corrupts per-row *slopes* (§6). A **mask-area guard** drops any detection whose bbox exceeds 15 % of the frame; on the bare-vine March bag the largest legitimate detection (a close trellis pole) is ~10.5 % of frame, so the guard rejects only a gross F007-style whole-frame blob without dropping real poles (CP-3 finding), while the median ± 0.5 m inlier test rejects any single off-row spurious base point. Output: left/right row positions `Y_L`, `Y_R` (each valid / invalid → two-row / single-row / none).
6. **Centreline, offset & heading (CP-3-locked — D035).** The centreline is the **midpoint of the left/right row medians, binned by range** — computed in the **1–3 m** bin (look-ahead ≈ 2 m, primary) and the **3–5 m** bin (≈ 4 m). The projection fan is *symmetric* (left row fans +Y, right row −Y with range), so it **cancels in the centreline midpoint**, leaving both quantities fan-free:
   - **lateral offset** `d̂_i` = signed Y of the centreline at the 1–3 m (≈ 2 m) look-ahead bin — **D-E** primary (+Y = left); 0 m / 3 m variants follow by re-binning. Metres.
   - **heading** `ψ̂_i` = angle of the centreline *direction between the 2 m and 4 m bins*, degrees. Per-row slopes are fan-corrupted and are **not** used — the heading is derived from the centreline precisely because the fan cancels there (D035).
...
- **(GT-2) Heading error — COMPLEMENTARY (adopted; redefined at CP-3, D035), always reported alongside GT-1.** `ψ̂_i` is the **centreline-derived** heading — the direction of the estimated centreline between its 2 m and 4 m look-ahead points (§4.6) — *not* a per-row slope, which the projection fan corrupts (per-row fits give spurious ±45° slopes; the fan cancels only in the centreline midpoint). Because the centreline is expressed in `base_link` (whose orientation is the robot's `/robot_pose` heading), **GT-2 heading error = √(mean_i ψ̂_i²)** in degrees under the same teleoperator-centred convention as GT-1 — i.e. `ψ̂_i` compared against the robot's actual heading from `/robot_pose` orientation, with the robot's own row-misalignment a common floor shared by all three arms (cancels in the cross-arm ranking). The slip-corrected form (subtract the path-tangent-minus-heading term, ≈ 0 for this forward-driving robot) is a documented refinement. Assumption-light orientation cross-check; adds no scope.
...
- **CP-3 — Single-arm dry run. [DONE]** Full pipeline (Phase C seed 42) over all **4 708 val frames**. Locked the row model (near-5 m Y-constant, ≥ 3 pts within 0.5 m; D035) and the fan-free centreline offset/heading. **Two-row coverage 64.0 %** (single-row 26.7 %, none 9.2 %); GT-1 offset SD 0.144 m (|median| 0.167 m, mean +0.164 m); GT-2 heading unbiased (mean −0.32°, |median| 3.21°, SD 7.73°). **Blob finding:** the F007 canopy-blob pathology does *not* manifest on bare-vine March frames — the largest detections (~10.5 % of frame) are **real close-up poles** (verified visually), so the 15 %-area guard drops 0 real detections; a naïve line-fit pipeline gave only 10 % two-row coverage because the fan destroys per-row line fits, which is why the row model is Y-constant and the heading is centreline-derived. Module `scripts/cp3_pipeline.py`; report `results/geometric/march/cp3_dryrun_report.json`; overlays `cp3_samples/`. Gate: passed.
...
- **D-F — Ground-truth metric. [LOCKED; GT-2 refined at CP-3 — D035]** GT-1 teleoperator-centred lateral (primary) + GT-2 heading (complementary, reported alongside); GT-3 LiDAR-referenced deferred to future work. GT-2 is the **centreline-derived** heading (2 m→4 m look-ahead), fan-free, replacing the fan-corrupted per-row slope. Rationale in §5; pipeline lock in D035 (DECISIONS.md).
