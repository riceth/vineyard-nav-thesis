...
> **Amendment (20 July 2026, additive — CP-P2 wiring validation; the P-2a decision stands, this fixes a *notation* mismatch).** The P-2a formula is written with a **leading minus** (`ω̂_i = −(Kp·offset_i + Kψ·heading_i) …`) under an error-sign convention. The **validated CP-P2 implementation uses `SIGN = +1` (corrective)** — `ω̂_i = +(Kp·offset_i + Kψ·heading_i + Kd·ḋ + Ki·∫offset)` — so that **offset > 0 (centreline to the robot's left, +Y) yields a positive/left yaw-rate steering toward centre**, empirically confirmed in the CP-P2 dry-run spans (offset +0.22 → ω +0.06; offset −0.42 → ω −0.06). **Read the leading-minus notation above as `SIGN = +1`.** The sign is a single documented constant in `scripts/control/command_generator.py`, not a re-derivation, and affects neither the wiring nor the §5.1 double-counting note.
...
> **Amendment (20 July 2026, additive — CP-P3 result F027).** This tracking metric is **demoted to a caveated diagnostic**: it is **not** a primary result and **cannot rank arms**. F027 showed the executed yaw-rate is essentially unpredictable from the centreline (best-possible linear R² = **0.0070**; per-arm out-of-fold RMS spread **0.004 °/s**), because the BLT reference was **GPS/topologically navigated**, not visually row-following. Report it only to quantify the **deployment/reference gap**, always carrying that caveat. **D014's strand-3 claim — "PID command smoothness — cross-arm comparable" — is unaffected:** §7(b) smoothness is computed on the command stream itself and needs **no** external reference, so §7(b)/(c)/(d) carry the cross-arm comparison. If this diagnostic is reported, use the **sign-corrected IMU** (`−/imu/data.angular_velocity.z`) — the IMU z-axis is inverted relative to `base_link` (F026 amendment).
...
- **P-6 — Output-conditioning parameters → all three sub-choices locked. [LOCKED]**
  ...
  - **Amendment (20 July 2026, additive — CP-P2): the `ω_max` clamp source is locked to the IMU gyro, not odom.** The clamp is `ω_max` = **IMU-gyro in-row p99 = 0.0606 rad/s (3.47 °/s)** (`/imu/data.angular_velocity.z`), **not** the `/odometry/base_raw.twist.angular.z` p99 named in the bullet above. **Justification (F026):** the odom twist yaw-rate is **noise-inflated and unreliable** (in-row p99 **13.4 °/s**, max **103 °/s**) versus the IMU gyro's clean signal (p99 **3.5 °/s**, max **7.6 °/s**) — using the noisier signal for a *safety clamp* would contradict the strand's own validated finding. The odom-based value is recorded for reference in `command_dryrun_summary.json`. (The "executed yaw-rate" *tracking reference* of §7a is a separate quantity, logged as both `omega_exec_odom` and `omega_exec_imu` for the CP-P4 choice — this amendment concerns only the P-6 clamp.)
...
> **Amendment (20 July 2026, additive — CP-P3 result F027; the 4b *procedure* was executed and is retained as evidence, but its *objective* is retired).** Running the k-fold as locked showed the objective `minimise RMS(ω̂ − ω^exec)` is **degenerate**: all 11 folds collapse to the smallest non-zero `Kp` with `Kψ = Kd = Ki = 0`, and the pooled out-of-fold RMS beats **commanding nothing at all** by **0.378 %**; the best-possible linear map (offset, heading) → ω^exec explains **R² = 0.0070** of the variance, and per-arm out-of-fold RMS differs by only **0.004 °/s** (arms indistinguishable, so the metric cannot serve D031). **Cause:** the BLT reference ran **GPS/topological navigation**, not vine-row visual servoing, so its steering cannot correlate with the perceived centreline — and **no choice of reference sensor repairs this** (odom, IMU and pose-derived all give R² ≈ 0; the problem is the reference *behaviour*). **Gain selection therefore pivots to P-4c (fixed principled gains, not data-fitted)** — already scoped above as an alternative, and it additionally removes the circularity 4b existed to solve (nothing is fitted, so there is nothing to be circular about). The 4b machinery (`scripts/control/gain_kfold.py` → `gain_kfold.json`) is retained as the **evidence for F027**, not as a tuning result. Pass-level folds remain the right construct if a valid objective is ever available. See **F027**.

- **P-4c — Gain selection: first-principles design point. [LOCKED 20 July 2026]** (supersedes 4b as the *route* to gains; 4b's machinery is retained only as F027 evidence). Plant model — unicycle, small angles, `offset ≈ −y`, `heading ≈ −ψ` — closes to **`ÿ + Kψ·ẏ + v·Kp·y = 0`**, so **ω_n = √(v·Kp)** and **ζ = Kψ/(2√(v·Kp))**; the heading term *is* the damping (§5.1, made rigorous). **Locked design point: ζ = 1.0** (critically damped — overshoot risks clipping a vine; correction speed is secondary) and **2 %-settling distance d_s = 20 m** of travel. **Rationale (measured, `command_summary.json → design_point_sensitivity`):** d_s = 5 m and 10 m saturate `ω_max` at offsets of **0.059 m / 0.234 m** — at or below the in-row offset RMS of **0.206 m**, i.e. the controller would hit full authority on ordinary F013-level estimation noise; **d_s = 20 m saturates only beyond F023's p99 offset tail (0.71 m vs its 0.937 m threshold)**, giving proportional response across the range that actually occurs; d_s = 30 m under-corrects within a single ~53 m row. A 5 m point would additionally demand **≈ 10.2× the observed in-row yaw-rate SD**, which the platform never used. **Resulting locked gains** at the measured v = 0.7687 m/s: **Kp = 0.064645 rad/s·m⁻¹, Kψ = 0.0077811 rad/s·deg⁻¹, Kd = 0, Ki = 0** (ω_n = 0.2229 rad/s). **`Kd = 0`** — `d(offset)/dt ≈ −v·ψ` duplicates the heading term (§5.1) and injects differencing noise; **`Ki = 0`** — an integrator chases the F017/F016 systematic bias, not a real error (CP-P2 showed wind-up to the clamp).
- **P-4c tilt de-bias. [LOCKED 20 July 2026]** The **F017/D038 sensor-common tilt of +2.31°** is subtracted from `heading` before the control law, as a **fixed calibration constant sourced from an independent, already-validated finding** — *not* a tuned parameter, so it reintroduces no circularity. Uncorrected it would permanently consume **29.7 % of `ω_max`** on a projection artefact rather than genuine row-centring error. **Scope:** heading only. The *offset* is **not** de-biased, because F016 shows its bias is **direction-dependent** (no single global constant is well-founded, and fitting one would edge into tuning); the tilt's ≈8 cm contribution at the 2 m look-ahead is therefore a documented residual. See **F027-A**.
- **P-6 ramp/rate limit — derived and locked. [LOCKED 20 July 2026]** (completes the P-6 entry above, whose value was previously left open.) The limit is set to **the maximum slew the locked closed-loop design can legitimately demand inside its own clamped envelope** — anything faster is not a control response but a perception jump or hold-transition step, which is what this layer exists to clip. Differentiating `ω = Kp·offset + Kψ·heading` (Kd=Ki=0) under the same plant model used for the gains (`d(offset)/dt = −v·ψ`, `d(heading)/dt = −ω`) gives `ω̇ = −ω_n²·ψ − 2ζω_n·ω`, hence `|ω̇| ≤ ω_n²·|ψ|max + 2ζω_n·ω_max`. Both bounds are already locked (`|ω| ≤ ω_max`; `|ψ| ≤ ω_max/Kψ_rad = ω_max/(2ζω_n)`, the heading at which the heading term alone saturates the clamp), collapsing to a closed form in **locked constants only**: **`ω̇_max = (2ζ + 1/(2ζ))·ω_n·ω_max`** = **2.5·ω_n·ω_max** at ζ=1 = **0.033743 rad/s²**. Implemented as a **derived expression** (`derive_ramp_rate()`), not a literal, so it tracks the design point if ω_n/ζ/ω_max ever change. **Measured effect** (`command_summary.json → ramp_layer`): demanded slew p50 0.0222 / p90 0.0955 / p99 0.8252 / max 1.998 rad/s²; the limiter clips **37.9 %** of transitions. The median sits *below* the bound (genuine transitions untouched); the p99 is **24.5×** it — perception jitter, not control. **CAVEAT (scope of validity):** the bound rests on **D038's straight-row model**, under which there is **no legitimate curvature term** for the limiter to clip. If the pipeline is extended to **curved-row** scenarios, a genuine curvature-driven slew term appears and **this bound must be re-derived, not assumed to still hold**. See **F027-A**.
