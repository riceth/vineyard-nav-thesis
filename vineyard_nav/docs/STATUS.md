...
**Title:** Multiclass Instance Segmentation for In-Row Vineyard Navigation: A Controlled Comparison Against the Binary-Mask Baseline
...
- **Confirmed on may (canopy; each spot-checked against its artefact):**
  ...
  - **F007** — geometric blob pathology **absent** (0 / 416k detections > 15% guard). *[written, F007 amendment]* **Neutral / not per-bag:** F014 (adjacent logging arm-invariant, scene-driven) holds; F016 neutral; F015/F019 superseded; **F025 — canopy near-seed sensitivity since re-run on may (27 Jul 2026): the optimum widens to 7.0–7.5 m vs march's 6.5 m, recovering 25% of abstentions vs 32%; may's higher baseline RMS softens the tolerance** *[written, F025 amendment]*.
...
### June multi-bag evaluation — COMPLETE

Fourth seasonal bag (`kg_june_08`, the **second canopy** bag). Locked pipeline, one whole-bag run; `check_bag_complete.py --bag june` gates completion.

- **Done:** conversion → CP-0 census (**D048 gate FIRED** — first bag: 88/90 unattributed scenes present, resolved by the two-stage rule + 6 visual confirmations) → CP-1 manifest (**7,308 in-row eligible**, 1,229 frames excluded as contamination, 7.6%) → frame extraction → detection cache → 9-model inference → in-row analyses (Stage C) → non-in-row (Stage D) → mitigation → **13** report figures (Stage E; turn category withheld, see below) → control strand (Stage F).
- **Confirmed on june (canopy, 2nd canopy bag; each spot-checked against its artefact):**
  - **F013 (headline)** — GT-1 and GT-2 **arm-indistinguishable on every pair**: A–B −2.3 mm CI[−5.4, +1.4], A–C −0.4 mm CI[−3.8, +2.8], B–C +1.7 mm CI[−0.1, +2.9]; GT-2 all CIs include zero; signs inconsistent across seeds on all three pairs. **F019 check finds no GT-2 pair excluding zero** at either the primary (L=37, 0.1°) or stricter (L=38, 0.05°) threshold — the **cleanest cross-arm null of the four bags** (march had a B–C micro-exclusion).
  - **F010** — tilt arm-consistent (A/B/C 2.75 / 2.81 / 2.79°, spread 0.07°), canopy-elevated vs bare-vine (march 2.10–2.18 / april 1.85–1.86) but **below may's 3.73–3.74°** — canopy raises tilt, not monotonically. **F011** — two_row coverage **67.8 / 64.9 / 61.5%**, lowest of the four bags. **F012** — GT-1 RMS 0.192–0.202 m, GT-2 3.15–3.30°.
  - **F017** — camera ↔ LiDAR tilt **agree in sign and size** (cam +2.36°, LiDAR +2.09° SD 0.53, Δ +0.27°) → sensor-common, not a camera artefact. **F018** — **no viable downstream config** (max 61.5% < 70% floor), second bag with none. *[F029 written]*
  - **F020** — spurious two_row on only **13.6–15.4%** of non-in-row frames (march 48–52 / april 20–32 / may 26–27) — the **deployment gap narrows under canopy**. **F021** — driven-path error **0.263–0.282 m / 5.09–5.45°**, the lowest lateral of the four bags. *[F029 amendment]*
  - **F022** — state gate collapses to **64.9 / 65.0 / 62.4%** @ 1.1–1.2% in-row FP; per category (arm A) stationary **100%**, turn 69.1%, transition **48.0%**, and june's spurious fits are **65.6% transition**. *[F022 amendment: motion-vs-place]* **F023** — geometry filter **57.5 / 43.2 / 46.9%** but at **48.6 / 22.9 / 27.5% in-row FP**, of which the `n_base` criterion alone contributes **47.2 / 20.9 / 25.8 pp** — the fixed `n_base_min = 12` exceeds june's mean in-row base count (10.5–12.3). **Not usable standalone on this bag.** *[F023 amendment: threshold-transfer failure]*
  - **F024** — in-row abstention **20.8 / 23.2 / 25.9%** single_row (canopy-elevated; march 12.8 / april 14.9 / may 22.0); dominant cause `too_few_near_seed` 63.7–70.4%, with the failing row still **detected on 92.3–93.6%** of those frames → starvation, not blindness.
  - **F026** — the **deployable** native gate (v_x > 0.337 m/s) **collapses in the same way**: reject **54.4 / 55.5 / 51.8%** @ 0.7% in-row FP (march 97.5 / april 87.7–92.3 / may 93.7–94.1), with transition at **35.7%** (march 91.9 / april 72.0 / may 83.4). Both turn predicates (odom_yaw +9 rejections/+68 FP; imu_yaw +18/+71) were **dropped** again, so the gate stays a single forward-speed predicate. **This independently confirms the F022 motion-vs-place limitation on the signal that would actually ship.**
  - **F027** — the open-loop objective is **degenerate on june too**: improvement over commanding *nothing* is **+0.177%** (IMU reference) and **−0.108%** (odometry — the first *negative* value across the four bags, i.e. the fitted gains are worse than zero gain); gain grid flat, Kp collapsing to a single distinct value across all 11 folds; per-arm OOF RMS 0.02615–0.02626 → **arms indistinguishable**. Sensor cross-check: odom-vs-IMU yaw **corr −0.417**, odom p99 12.28 vs IMU 4.19 °/s.
  - **F028** — command smoothness: on the **raw** command A–B resolves (+0.00041 [+0.00003, +0.00138]) and, on the exclusive view, A–B (+0.00096) and A–C (+0.00142) — the **same arm-A-specific direction as may**; but the **ramped (deployed) command is null on every pair, both views**, as on may. Blob-seed contrast remains sign-inconsistent (B +0.00058, C −0.00067) → no blob effect.
  - **F007** — june is the **first bag with a geometric-stream blob**: exactly 1 detection of 252,742 above the 15% guard (seed 42, frame 7460, `trunk`, conf 0.28, **27.03% of frame**), visually confirmed as the 6799 whole-canopy pathology, **dropped by the D035 guard before base-point extraction**. *[F007 amendment; rendered artefact committed]*
  - **F029 (new)** — canopy finding: the class-structure mechanism is **season-invariant** (pole supplement +13.7 pp vs march's +12.4; trunk-only CI-indistinguishable from agnostic on all four bags) while **base-point availability collapses** (30.3 → 11.5 pts/frame), taking coverage below the deployability floor. Answers F018's registered open question. *[written]*
  - **D048** — first bag where the gate **fired**: 88 of 90 unattributed scenes **present**, 1,229 frames (7.6%) excluded as perception-training contamination, resolved by the two-stage rule + 6 visual confirmations. *[D048 amendment; `d048_confirmed.json` committed]*
  - **Figures** — **13, not 15**: june's `turn` category is **withheld** (all 40 of its turn frames contain identifiable people; march/april/may screened clean). Only the imagery is withheld — turn statistics remain in the fig9/fig10 footers and `mitigation_analysis.json`. *[documented in README, FIGURE_SPEC §5, `FIGURE_EXCEPTIONS`]*
...
`kg_september_09` was converted and taken through CP-0/CP-1. The D048 gate is **clean** (0/90 present). CP-1 yields **250 eligible frames of 28,202 (0.9%)** in twelve 1–3 second "passes". Root cause: **`/health/gps/error_std` p90 15.20 m / max 41.96 m, 25.8% of the session above 1 m** — the RTK fix was lost for roughly a quarter of the recording, so `/robot_pose` degenerates to a 0.26 Hz step function with snaps up to 18.84 m, and the twelve passes are pose jumps rather than traverses. Because the pose **is** the driven-path reference (yardstick: the 3.8 cm RTK floor), this is unrecoverable by any frame-selection change. See **D051**.

**Evaluated set: march / april / may / june / july2023** — five geometric, four control. july-2022 excluded by D050, september by D051, august2023 by D054 (the session's camera recorded no imagery — all 8,916 frames one blank white JPEG). july2023 is evaluated **in-row only** and carries a narrowed reporting scope: its A–B/A–C GT-1 divergence and coverage separation are reported as a domain-generalisation observation, while B–C and A–C GT-2 are withheld as too imprecise to report in either direction — see **D052** and **D053**.
...
**Comparisons:**
- A ↔ B → **pipeline-generation effect** at fixed binary labelling — **not** an architecture effect; the arms differ in ≥ 13 respects (D006/D021), so nothing is attributable to architecture alone
- B ↔ C → **class-structure effect** at fixed YOLO architecture — **the one controlled comparison**
...
3. **Commit 6 (O014) — DONE** — non-in-row: `--scope` added; 5,841 category-C frames → `final/non_in_row_evaluation/`; F020 (spurious `two_row` ~48–52%, A 76.0 / B 80.5 / C 78.7 % on turns) + F021 (driven-path error ~0.40 m / ~6°, ~2× in-row, IPM-invalid).
...
- **O021 (documentation correction — RESOLVED 8 Aug 2026):** Six documents described **A ↔ B as isolating an architecture effect**. This is inaccurate and would have propagated into the dissertation: arms A and B differ in at least **thirteen** respects — architecture, pre-training corpus, segmentation paradigm, optimiser, learning rate (100×), weight decay, schedule, epochs, batch size, loss, early-stopping patience, augmentation policy, and output representation — verified against `configs/phase_a_unet_binary.yaml` and `configs/phase_b_yolo_binary_train.yaml`. **Only B ↔ C is controlled.** Corrected following the O020 additive pattern (correct in place, note the supersession, do not silently rewrite): additive corrections at `DECISIONS.md` D006 and D021 with an inline pointer at D032; in-place corrections at `PHASE_B_SPEC.md:6` and `PROJECT_PLAN.md:17,140`. `FINDINGS.md:443` and `PHASE_C_SPEC.md:6` were checked and are **correct as written** — both describe B ↔ C, which genuinely is controlled. Consequence for the write-up: no architecture-attributable claim may rest on A ↔ B; it is a baseline-versus-modernised-pipeline contrast.
- **O020 (documentation correction — RESOLVED 27 Jul 2026):** The pre-CP-2-era docs — `PROJECT_PLAN.md` (§ metrics table, reproducibility, limitations), `GEOMETRY_PIPELINE_SPEC.md`, `PHASE_C_SPEC.md`, `PID_PIPELINE_SPEC.md`, `CONTROL_DESIGN_INTENT.md`, `DECISIONS.md` — still describe the BLT reference trajectory as **"teleoperator/teleoperated"**. This is a **mischaracterisation**: the platform ran **autonomous GPS/topological navigation** (Polvara et al. 2024 §3.3.3), established by F016 (driven-path framing) and made load-bearing by F027 (the recorded steering is uncorrelated with vision because the platform never steered from vision — R²≈0.007). The new top-level `README.md` uses the corrected term ("autonomous driven-path") and flags the discrepancy in its Honest-notes section. **Required:** an additive-amendment pass over those docs replacing "teleoperator/teleoperated" with the autonomous-driven-path characterisation, following the same additive-preservation pattern as the F017 anchor-selector amendment (correct in place, note the supersession, do not silently rewrite history). **Resolved 27 Jul 2026:** `PROJECT_PLAN.md` (§5 metrics table, §6 selection rule, §9 limitation) and `PHASE_C_SPEC.md` corrected in place to the autonomous driven-path characterisation (D014/Polvara 2024 cited); `GEOMETRY_PIPELINE_SPEC.md` carries an additive terminology amendment at the D-F convention (the "teleoperator-centred" convention *name* retained to match committed artefacts, characterisation corrected → "driven-path-centred"); `DECISIONS.md`, `PID_PIPELINE_SPEC.md` and `CONTROL_DESIGN_INTENT.md` already carried the D014-amendment correction (verified). Top-level `README.md` and the Notion supervisor log also corrected.
