...
- **Remaining strand work (in order):** Commits 2a–6 done (pooling, pipeline consolidation, findings, STATUS + per-month template, manifest cleanup + whole-bag subsample, non-in-row characterisation F020/F021). Next: report figures (O012), then multi-bag seasonal (April+) and PID.
