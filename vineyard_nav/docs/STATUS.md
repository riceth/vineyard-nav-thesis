...
### April multi-bag evaluation — IN PROGRESS
...
- **Confirmed on april so far:** **F013 (headline) — GT-1 arm-indistinguishability holds in a second season** (≤ 3.2 mm, all CIs include zero; A–B closes *at* zero). **F010** — tilt arm-consistency holds at a different magnitude (SD 0.007°). **F011's** own bound vindicated (coverage 77%, not 83–84%). **F018** — class-agnostic re-locks; poles supplement coverage (+13.6 pp), pole-only degenerates (0.6%). **March's GT-2 sub-noise-floor offset does NOT reproduce** → bounded as march-specific.
- **Running:** F017 lidar cross-check, F024 abstention, F025 near-seed sensitivity.
- **Pending:** non-in-row extraction + inference (F020/F021, F022/F023, and F026's *full* validation — which is why non-in-row must precede the april control-strand re-run); then the control strand on april (F027, F027-A, F028).
