...
overlays go to the per-bag QA dir resolved by bag_config (small, committed; for march the
...
SAMPLES = B["qa_samples"]          # per-bag (march keeps its legacy committed location)
CAM = "/camera_link_rear/color/image_raw"          # raw Image, not CompressedImage
...
def main() -> None:
    ...
    def decode(i):
        """Riseholme publishes RAW sensor_msgs/Image (rgb8, 1280x720), where Ktima publishes a
        JPEG CompressedImage (bgra8, 1920x1080). Only the decode differs: the 640x640 STRETCH and
        the JPEG-90 write below are identical, so the frames reaching the models are prepared the
        same way at both sites."""
        data = cur.execute("SELECT data FROM messages WHERE id=?", (ids[i],)).fetchone()[0]
        m = TS.deserialize_cdr(bytes(data), "sensor_msgs/msg/Image")
        im = np.frombuffer(m.data, np.uint8).reshape(m.height, m.width, -1)
        if m.encoding == "rgb8":
            im = cv2.cvtColor(im, cv2.COLOR_RGB2BGR)
        elif m.encoding == "rgba8":
