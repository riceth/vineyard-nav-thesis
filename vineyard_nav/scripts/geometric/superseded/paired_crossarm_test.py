...
for ln in open(f"{PKG}/results/geometric/march/superseded/march_val_test_split/test_evaluation/line_fit_test_per_frame.csv").read().splitlines()[1:]:
...
json.dump({"per_arm_ci": {a: {"gt1_rms": rms([r[2] for r in arm_rows[a][0]]), "gt1_ci": block_stat_ci(arm_rows[a][0], L_GT1, rms),
           ...
           "paired": paired}, open(f"{PKG}/results/geometric/march/superseded/march_val_test_split/test_evaluation/paired_crossarm_test.json", "w"), indent=2)
