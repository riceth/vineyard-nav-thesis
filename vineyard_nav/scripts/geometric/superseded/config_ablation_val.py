...
import numpy as np
PKG = Path(__file__).resolve().parents[3]; sys.path.insert(0, str(PKG/"scripts"/"geometric")); sys.path.insert(0, str(PKG))
...
sw = json.load(open(PKG/"results/geometric/march/superseded/march_val_test_split/val_evaluation/config_sweep_val.json")); sw.update(out)
json.dump(sw, open(PKG/"results/geometric/march/superseded/march_val_test_split/val_evaluation/config_sweep_val.json", "w"), indent=2)
