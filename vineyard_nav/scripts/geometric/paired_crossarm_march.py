"""March pooled paired cross-arm difference bootstrap (D040, F013).
Consolidates paired_crossarm_val.py + paired_crossarm_test.py on the pooled both-two-row frames.

For each arm pair (A-B, A-C, B-C), on frames where BOTH arms produced a two-row estimate,
per-frame paired differences ΔGT-1 = offset_X - offset_Y (m), ΔGT-2 = heading_X - heading_Y (deg):
  across_seed : per-frame mean over the 3 seeds of each arm (two-row in all 3 of both arms),
                point mean Δ + moving-block bootstrap 95% CI  (primary).
  per_seed    : X_s vs Y_s per seed, point mean Δ + block CI + sign  -> sign consistency.
...
Block lengths (L_GT1, L_GT2) are RE-DERIVED on the pooled data via block_lengths.py
(Analysis-H, 2× decorrelation, conservative across pairs); the val values were 11/31 and are
NOT hardcoded here (POOLING_SPEC #2). val+test pass_ids are disjoint, so blocks stay within a
single contiguous pass.

Writes results/geometric/march/final/march_evaluation/paired_crossarm_march.json.
"""
import sys
import json
import collections
...
OUT = PKG / "results/geometric/march/final/march_evaluation/paired_crossarm_march.json"

# path position (pass, cumulative m) per pooled val+test eligible frame within its pass
pos = {}
...
report = {"config": {"paired_on": "both-arms two_row (pooled val+test)",
...
print(f"pooled paired cross-arm; block lengths L_GT1={L_GT1} L_GT2={L_GT2}")
print("\n=== across-seed (point mean Δ [moving-block 95% CI]; %=of floor) ===")
for pk, a in across_seed.items():
    g1, g2 = a["GT1"], a["GT2"]
    print(f"{pk} (n={g1['n']}): "
          f"GT1 Δ={g1['mean_diff'] * 1000:+.1f} mm ({g1['mean_diff_pct_floor']:+}% RTK) "
    ...
    print(f"      GT2 Δ={g2['mean_diff']:+.3f}° ({g2['mean_diff_pct_floor']:+}% floor) "
          f"CI{g2['ci95']}° excl0={g2['ci_excludes_zero']}")
