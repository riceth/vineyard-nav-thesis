"""March pooled Phase C downstream config analysis (D040, F018).
Consolidates config_sweep_val.py + config_ablation_val.py + config_ablation_test.py.

Reads the POOLED detection cache (CACHE_DIR/detections_val.csv + detections_test.csv,
concatenated -- val/test frame indices are disjoint) and, over the pooled frames, runs:
  1. the full downstream config sweep: class-agnostic + trunk-primary / pole-primary × T∈{1,2,3,5,8,12},
...
GT-1/GT-2 RMS + moving-block bootstrap CI at the pooled Analysis-H block lengths (block_lengths.py,
re-derived on pooled data -- NOT the val 11/31), mean base points.

The class-agnostic design decision is LOCKED (F018) and re-reported on pooled data, not
re-selected. argmin_summary flags agnostic_locked; if a non-agnostic cell strictly wins (CI does
not overlap agnostic) pause_flag=True and the pooling should pause for review.

Writes results/geometric/march/final/march_evaluation/config_analysis_march.json.
"""
import sys
import json
import collections
...
DETS = [CACHE_DIR / "detections_val.csv", CACHE_DIR / "detections_test.csv"]
PF = PKG / "results/geometric/march/final/march_evaluation/line_fit_march_per_frame.csv"
OUT = PKG / "results/geometric/march/final/march_evaluation/config_analysis_march.json"
SEEDS = [42, 43, 44]
T_GRID = [1, 2, 3, 5, 8, 12]
...
def run_cell(selector):
    """selector(dets) -> base_pts. Returns the pooled across-seed cell record."""
...
out = {"config": {"n_frames": len(FRAMES), "seeds": SEEDS, "T_grid": T_GRID,
                  "upstream": "line-fit locked (D035-D038); conf 0.25, 15% blob guard",
                  "boot_iters": BOOT, "block_lengths": bl,
                  "note": "class-agnostic locked (F018); re-reported on pooled data, not re-selected"},
